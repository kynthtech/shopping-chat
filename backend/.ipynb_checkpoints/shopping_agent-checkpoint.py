...
def search_products(query: str) -> List[Dict[str, Union[int, str, float]]]:
    ...
    try:
        cursor.execute("SELECT product_id, name, price, stock FROM products WHERE name LIKE ?;", ('%' + query + '%',))
        products = cursor.fetchall()
...
def add_to_cart(user_id: int, product_id: int, quantity: int) -> str:
    ...
    try:
        # Get product details
        cursor.execute("SELECT product_id, name, stock FROM products WHERE product_id=?;", (product_id,))
        product = cursor.fetchone()
...
tools = [list_products , product_details, search_products, add_to_cart, view_cart, checkout, get_order_status]
